...
SESSION_JSON=$(curl -sS -X POST "$BASE/work" -H 'Content-Type: application/json' -d "$CREATE_PAYLOAD")
...
STATUS_JSON=$(curl -sS "$BASE/work/$SESSION_ID")
...
OUT_JSON=$(curl -sS "$BASE/work/$SESSION_ID/outputs")
...
INP_JSON=$(curl -sS -X POST "$BASE/work/$SESSION_ID/input" -H 'Content-Type: application/json' -d "$INP_PAYLOAD" || true)
echo "$INP_JSON"
