...
async fn register_user(app: &TestApp, username: &str, password: &str, email: Option<&str>) -> actix_web::dev::ServiceResponse {
...
async fn login_user(app: &TestApp, username: &str, password: &str) -> actix_web::dev::ServiceResponse {
    let req_data = json!({
        "username": username,
        "password": password
    });
...
async fn test_login_valid_credentials_returns_token() {
    ...
    let token = login_response["token"].as_str().expect("Token should be present in response");
...
async fn test_registration_duplicate_username_returns_error() {
    ...
    let response = register_user(&app, "alice", "AnotherPass456!", Some("alice2@example.com")).await;
...
async fn test_jwt_token_expiration_24_hours() {
    ...
    let claims = nocodo_manager::auth::validate_token(_token, "test_jwt_secret_key_for_auth_tests").unwrap();
