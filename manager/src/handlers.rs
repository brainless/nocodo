...
use crate::templates::{TemplateManager, ProjectTemplate};
...
pub async fn create_project(
    ...
    if project_path.exists() {
        return Err(AppError::InvalidRequest(
            format!("Project directory already exists: {}", project_path.display())
        ));
    }
...
", req.name);
    ...
    tracing::info!("Successfully created project '{}' at {}", project.name, project.path);
    ...
    for file in &template.files {
        ...
        if let Some(parent) = file_path.parent() {
            std::fs::create_dir_all(parent)
                .map_err(|e| AppError::Internal(format!("Failed to create directory {}: {}", parent.display(), e)))?;
        }
        
        // Process template content
        let processed_content = handlebars.render_template(&file.content, &context)
            .map_err(|e| AppError::Internal(format!("Template processing error: {}", e)))?;
        
        // Write the file content
        std::fs::write(&file_path, processed_content)
            .map_err(|e| AppError::Internal(format!("Failed to write file {}: {}", file_path.display(), e)))?;
        ...
        if file.executable {
            ...
            std::fs::set_permissions(&file_path, perms)
                .map_err(|e| AppError::Internal(format!("Failed to set permissions for {}: {}", file_path.display(), e)))?;
        }
    }
