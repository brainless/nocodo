...
pub async fn handle_embedded_file(req: HttpRequest) -> ActixResult<HttpResponse> {
    ...
    match WebAssets::get(file_path) {
        Some(content) => {
            ...
            let mut response = HttpResponse::Ok().content_type(mime.as_ref());
            
            // Add caching headers for static assets (not for HTML)
            if !file_path.ends_with(".html") {
                response = response
                    .insert_header(("Cache-Control", "public, max-age=86400")) // 24 hours
                    .insert_header(("ETag", format!("\"{}\"", content.len())));
            } else {
            ...
            match content.data {
                Cow::Borrowed(bytes) => Ok(response.body(bytes)),
                Cow::Owned(bytes) => Ok(response.body(bytes)),
            }
        }
        None => {
            tracing::debug!("Embedded file not found: {}, serving index.html for SPA routing", file_path);
...
pub fn list_embedded_files() -> Vec<String> {
    WebAssets::iter().map(|f| f.to_string()).collect()
}
...
pub fn get_embedded_assets_size() -> usize {
    WebAssets::iter()
        .filter_map(|path| WebAssets::get(&path))
...
pub fn configure_embedded_routes(cfg: &mut web::ServiceConfig) {
    cfg
        ...
        .route("/{filename:.*\\.(js|css|ico|svg|png|jpg|jpeg|woff|woff2|ttf)}", 
...
mod tests {
    ...
    fn test_embedded_assets_exist() {
        ...
        if files.is_empty() {
            println!("Warning: No embedded assets found. Run 'cd manager-web && npm run build' first.");
            println!("This is expected during development.");
        } else {
            println!("Found {} embedded assets", files.len());
            assert!(files.iter().any(|f| f == "index.html"), "index.html should be embedded");
        }
    }
    ...
    fn test_asset_validation() {
        ...
        assert!(is_valid || cfg!(debug_assertions), 
