...
impl Database {
    ...
    fn run_migrations(&self) -> AppResult<()> {
        ...
        // Drop legacy LLM agent tables (removed in cleanup)
        let _ = conn.execute("DROP TABLE IF EXISTS llm_agent_tool_calls", []);
        let _ = conn.execute("DROP TABLE IF EXISTS llm_agent_messages", []);
        let _ = conn.execute("DROP TABLE IF EXISTS llm_agent_sessions", []);
