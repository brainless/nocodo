use crate::helpers::agents::create_imap_agent;
...
#[post("/agents/imap/execute")]
pub async fn execute_imap_agent(
    ...
    let (host, port, username, password) = match &req.config {
        ...
        _ => {
            error!(config_type = ?req.config, "Invalid config type for IMAP agent");
            return HttpResponse::BadRequest().json(ErrorResponse {
                error: "Expected IMAP agent config".to_string(),
            });
        }
    };
    ...
    info!(
        ...
        port = port,
        username = %username,
        "Executing IMAP Email agent"
    );

    let user_prompt = req.user_prompt.clone();
    let agent_name = "imap".to_string();
    ...
    tokio::spawn(async move {
        let agent = match create_imap_agent(
            &llm_client_clone,
            &storage_clone,
            &host,
            port,
            ...
            Ok(agent) => agent,
            Err(e) => {
                error!(error = %e, session_id = session_id, "Failed to create IMAP agent");
                let mut session = Session {
                    id: Some(session_id),
                    agent_name: "imap".to_string(),
                    provider,
                    model,
                    system_prompt: None,
