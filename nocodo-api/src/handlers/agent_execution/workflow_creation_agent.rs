use crate::helpers::agents::create_structured_json_agent;
...
#[post("/agents/workflow-creation/execute")]
pub async fn execute_workflow_creation_agent(
    ...
    let (type_names, domain_description) = match &req.config {
        ...
        _ => {
            ...
            return HttpResponse::BadRequest().json(ErrorResponse {
                error: "Expected Structured JSON agent config".to_string(),
            });
        }
    };
    ...
    info!(
        ...
        "Executing Workflow Creation agent"
    );

    let user_prompt = req.user_prompt.clone();
    let agent_name = "workflow-creation".to_string();
    ...
    tokio::spawn(async move {
        let agent = match create_structured_json_agent(
            ...
            Ok(agent) => agent,
            Err(e) => {
                error!(error = %e, session_id = session_id, "Failed to create Workflow Creation agent");
                let mut session = Session {
                    id: Some(session_id),
                    agent_name: "workflow-creation".to_string(),
                    provider,
                    model,
                    system_prompt: None,
