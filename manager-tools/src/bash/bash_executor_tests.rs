...
mod tests {
    ...
    async fn test_bash_executor_nonexistent_command() {
        let permissions =
            BashPermissions::new(vec![PermissionRule::allow("nonexistent_command*").unwrap()]);
        ...
        assert!(
            execution_result.stderr.contains("not found")
                || execution_result.stderr.contains("command not found")
        );
    }
    ...
    async fn test_bash_executor_working_directory() {
        ...
        let result = executor
            .execute(&format!("ls {}", test_file.display()), Some(5))
            .await;
    ...
    async fn test_bash_executor_git_commands() {
        ...
        let init_result = executor
            .execute(&format!("git init {}", temp_dir.path().display()), Some(5))
            .await;
        ...
        let status_result = executor
            .execute(
                &format!("git -C {} status", temp_dir.path().display()),
                Some(5),
            )
            .await;
        ...
        let add_result = executor
            .execute(
                &format!("git -C {} add test.txt", temp_dir.path().display()),
                Some(5),
            )
            .await;
    ...
    async fn test_bash_executor_cargo_commands() {
        ...
        let init_result = executor
            .execute(
                &format!(
                    "cd {} && cargo init --bin test_project",
        ...
        let check_result = executor
            .execute(
                &format!("ls {}", temp_dir.path().join("test_project").display()),
                Some(5),
            )
            .await;
    ...
    async fn test_bash_executor_complex_command() {
        ...
        let result = executor
            .execute(
                &format!("cat {} | grep hello | wc -l", test_file.display()),
                Some(5),
            )
            .await;
