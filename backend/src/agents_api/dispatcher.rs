...
use nocodo_agents::{
    ...
    AgentType, ChatMessage, ContextAgentResponse, Epic, EpicStatus, SqliteAgentStorage,
    SqliteTaskStorage, Task, TaskStatus, TaskStorage, UiDesignerResponse,
};
...
async fn dispatch_task(event: DispatchEvent, db_path: String) {
    match event.assigned_to_agent.as_str() {
        ...
        "backend_context" => dispatch_backend_context(event, &db_path).await,
        "admin_gui_context" => dispatch_admin_gui_context(event, &db_path).await,
...
async fn dispatch_backend_context(event: DispatchEvent, db_path: &str) {
    dispatch_context_agent(event, db_path, "backend_context").await;
}

async fn dispatch_admin_gui_context(event: DispatchEvent, db_path: &str) {
    dispatch_context_agent(event, db_path, "admin_gui_context").await;
}

async fn dispatch_context_agent(event: DispatchEvent, db_path: &str, agent_type: &str) {
    let task_id = event.task_id;
    ...
    let result = if agent_type == "backend_context" {
        let agent = match nocodo_agents::build_backend_context_agent(&config, db_path, event.project_id, &project_path) {
            Ok(a) => a,
            Err(e) => {
        ...
        agent.run_for_task(session_id, task_id).await
    } else {
        let agent = match nocodo_agents::build_admin_gui_context_agent(&config, db_path, event.project_id, &project_path) {
            Ok(a) => a,
            Err(e) => {
                log::error!("[Dispatcher] {} task={} build error: {}", agent_type, task_id, e);
                return;
    ...
    match result {
        Ok(ContextAgentResponse::ContextSaved { context }) => {
            log::info!(
        ...
        Ok(ContextAgentResponse::Stopped(reason)) => {
