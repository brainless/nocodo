...
pub async fn generate_form(
    ...
    let table_def_json = match schema_storage
        .get_latest_schema_for_project(project_id)
        .await
    {
        Ok(Some(schema_json)) => match serde_json::from_str::<SchemaDef>(&schema_json) {
            Ok(schema) => match schema.tables.into_iter().find(|t| t.name == entity_name) {
...
#[get("/api/agents/ui-designer/entities/{project_id}")]
pub async fn list_entities(state: web::Data<AgentState>, path: web::Path<i64>) -> impl Responder {
...
#[get("/api/agents/ui-designer/forms/{project_id}")]
pub async fn list_forms(state: web::Data<AgentState>, path: web::Path<i64>) -> impl Responder {
    let project_id = path.into_inner();
        ...
        Ok(pairs) => {
            let forms = pairs
                ...
                .filter_map(|(entity_name, json)| {
                    serde_json::from_str::<FormLayout>(&json)
                        .ok()
                        .map(|layout| FormLayoutResponse {
                            entity_name,
                            layout: FormLayoutJson::from(layout),
                        })
                })
