...
//!
//! This module formats PostgreSQL query results into human-readable table format optimized for
...
//! - **Type-aware formatting**: Proper handling of NULL, numbers, strings, booleans, arrays, and JSON
//!
//! # Example Output
//!
//! ```text
...
pub fn format_query_result(result: &crate::postgres_reader::executor::QueryResult) -> String {
...
fn format_cell_value(value: &serde_json::Value) -> String {
    match value {
        ...
        serde_json::Value::Object(obj) => {
            // Format JSON objects in compact form
            let formatted = serde_json::to_string(obj).unwrap_or_else(|_| "{}".to_string());
            if formatted.len() > 50 {
                format!("{}...", &formatted[..47])
            } else {
                formatted
...
mod tests {
    use super::*;
    use crate::postgres_reader::executor::QueryResult;
