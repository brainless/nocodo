...
use crate::{error::CliError, commands};
use commands::*;
...
impl Cli {
    pub async fn run(&self) -> Result<(), CliError> {
        match &self.command {
            Some(Commands::Analyze { path, format }) => {
                self.handle_analyze(path, format).await
            }
            ...
            Some(Commands::Init { template, path }) => {
                self.handle_init(template, path).await
            }
            Some(Commands::Structure { action }) => {
                self.handle_structure(action).await
            }
    ...
    async fn handle_analyze(&self, path: &Option<PathBuf>, format: &Option<OutputFormat>) -> Result<(), CliError> {
    ...
    async fn handle_validate(&self, file: &PathBuf, language: &Option<String>) -> Result<(), CliError> {
