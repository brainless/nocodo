...
const ToolIcon: Component<{ toolName: string; className?: string }> = (props) => {
  ...
  return (
    ...
    >
      <span class="mr-1" aria-hidden="true">{getToolIcon(props.toolName)}</span>
  ...
  return (
    ...
    >
      <Show when={props.showIcon !== false}>
        <span 
          class={props.status === 'running' ? 'animate-spin' : ''} 
          aria-hidden="true"
        >
          {getStatusIcon()}
        </span>
        ...
        <div class="px-4 py-4 sm:px-6">
          ...
          <Show when={session().status === 'running'}>
            <div class="mt-2 flex items-center text-xs text-blue-600">
              <div class="w-1.5 h-1.5 bg-blue-400 rounded-full animate-pulse mr-2" aria-hidden="true"></div>
