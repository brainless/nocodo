...
const AGENT_TYPE: &str = "frontend_engineer";
const MAX_NUDGES: u32 = 8;

#[derive(Debug)]
pub enum FrontendEngineerResponse {
...
impl FrontendEngineerAgent {
    ...
    pub async fn run_for_task(
        ...
        session_id: i64,
        task_id: i64,
    ) -> Result<FrontendEngineerResponse, AgentError> {
    ...
    async fn run_loop(
        ...
        let list_files_tool = Tool::from_type::<ListFilesParams>()
            ...
            .description(
                "List files and directories at the given path relative to the project root. \
                 Pass an empty string for the project root, or a subdirectory path like 'admin-gui/src'. \
        ...
        let read_file_tool = Tool::from_type::<ReadFileParams>()
            ...
            .description(
                "Read the contents of a file at the given path relative to the project root. \
                 Use this to examine source files, package.json, config files, components, etc.",
            )
            .build();
        ...
        let system_prompt = super::prompts::system_prompt();
        ...
        loop {
                    ...
                    match tool_name {
                        ...
                        unknown => {
                            return Err(AgentError::Other(format!(
                                "frontend_engineer called unknown tool: {}",
                                unknown
                            )));
                        }
                    }
            ...
            if !assistant_text.is_empty() {
                ...
                if trimmed.starts_with('{') || trimmed.starts_with('[') {
                    self.context_storage
                        .save_context(self.project_id, AGENT_TYPE, trimmed)
                        .await?;
                    self.storage
            ...
            if nudges >= MAX_NUDGES {
                return Err(AgentError::Other(
                    "frontend_engineer did not produce context after multiple nudges".to_string(),
                ));
            }
            ...
            self.storage
                .create_message(ChatMessage {
                    ...
                    role: "user".to_string(),
                    agent_type: None,
                    content: "Continue exploring admin-gui files and respond with the complete JSON context summary as plain text only.".to_string(),
                    tool_call_id: None,
                    tool_name: None,
                    turn_id: None,
                    created_at: 0,
